...
class SafeService:
    def __init__(self):
        ...
        self.backend_account = Account.from_key(settings.backend_private_key) if settings.backend_private_key else None
    ...
    def propose_safe_tx(
        ...
        if not sender_address and not self.backend_account:
            raise ValueError("sender_address or BACKEND_PRIVATE_KEY is required to propose a Safe transaction")
    ...
    def execute_direct_eoa_tx(self, to: str, data: str, value: int = 0) -> Dict[str, Any]:
        if not self.backend_account:
            raise ValueError("BACKEND_PRIVATE_KEY is required for direct EOA execution")
